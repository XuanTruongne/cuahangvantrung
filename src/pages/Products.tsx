        ...
        <section className="section-padding bg-background">
          <div className="container-custom">
            ...
            {isLoading ? (
              ...
              <AnimatedContainer className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6" staggerDelay={0.05}>
                {paginatedProducts.map((product) => (
                  <AnimatedItem key={product.id}>
                    <Link
                      to={`/products/${product.slug}`}
                      className="group card-industrial block h-full"
                    >
                      {/* Image */}
                      <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
                        ...
                        <div className="absolute top-3 left-3 flex gap-2">
                          <span className="px-2 py-1 bg-primary text-primary-foreground text-xs font-medium rounded">
                            {(product.categories as { name: string } | null)?.name || "Khác"}
                          </span>
